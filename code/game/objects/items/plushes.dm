	...
	if(COOLDOWN_FINISHED(src, change_ninja_cooldown))
		...
		switch(current_skin)
			if(BLUE_NINJA_SKIN)
				...
				playsound(src, 'modular_bluemoon/sound/plush/Blue Ninja Plushie.ogg', 50, 1)
			if(RED_NINJA_SKIN)
				say("Ты можешь бежать... но ты не сможешь спрятаться!")
				playsound(src, 'modular_bluemoon/sound/plush/Red Ninja Plushie.ogg', 50, 1)
			else
				say("Я не боюсь тьмы! Я и есть тьма!")
				playsound(src, 'modular_bluemoon/sound/plush/Green Ninja Plushie.ogg', 50, 1)
	if(ismob(loc))
		var/mob/M = loc
		M.update_inv_hands()
	...
	if(COOLDOWN_FINISHED(src, change_miner_cooldown))
		...
		switch(current_skin)
			if(BASIC_MINER_SKIN)
				...
				playsound(src, 'modular_bluemoon/sound/plush/Miner Plushie.ogg', 50, 1)
			else
				say("Кишки, огромные кишки! Убей их… должен убить их всех! Разорвать… и… рвать! Демоны… они повсюду. Должен… убить их всех!")
				playsound(src, 'modular_bluemoon/sound/plush/Bloody Miner Plushie.ogg', 50, 1)
	if(ismob(loc))
		var/mob/M = loc
		M.update_inv_hands()
