...
/obj/machinery/computer/shuttle_flight
	...
	var/datum/action/innate/supercruise_docker_rotate/rotate_action = new
...
/obj/machinery/computer/shuttle_flight/proc/CreateEye()
	...
	eyeobj = new /mob/camera/aiEye/remote/supercruise_docker(null, src)
	var/mob/camera/aiEye/remote/supercruise_docker/the_eye = eyeobj
...
/obj/machinery/computer/shuttle_flight/proc/placeLandingSpot()
	...
	if(designate_time && (landing_clear != SHUTTLE_DOCKER_BLOCKED))
		...
		//WHITE-STEEL PORT: target обязан быть валидным атомом - NONE давал прогресс-бар на null-цели.
		var/wait_completed = do_after(current_user, designate_time, designating_target_loc, TRUE, CALLBACK(src, .proc/canDesignateTarget))
...
/mob/camera/aiEye/remote/supercruise_docker
	visible_icon = FALSE
	use_static = FALSE
	var/list/placement_images = list()
	var/list/placed_images = list()

/mob/camera/aiEye/remote/supercruise_docker/Initialize(mapload, obj/machinery/computer/camera_advanced/origin)
	src.origin = origin
	return ..()

/mob/camera/aiEye/remote/supercruise_docker/setLoc(T, force_update = FALSE)
	..()
	var/obj/machinery/computer/shuttle_flight/console = origin
	console.checkLandingSpot()

/mob/camera/aiEye/remote/supercruise_docker/update_remote_sight(mob/living/user)
	user.sight = BLIND|SEE_TURFS
	user.lighting_alpha = LIGHTING_PLANE_ALPHA_INVISIBLE
	user.sync_lighting_plane_alpha()
	return TRUE

/datum/action/innate/supercruise_docker_rotate
	name = "Поворот"
	icon_icon = 'icons/mob/actions/actions_mecha.dmi'
	button_icon_state = "mech_cycle_equip_off"

/datum/action/innate/supercruise_docker_rotate/Activate()
...
/datum/action/innate/camera_jump/shuttle_docker/Activate()
	...
	if(selected)
		...
		if(T)
			...
			C.overlay_fullscreen("flash", /atom/movable/screen/fullscreen/flash)
